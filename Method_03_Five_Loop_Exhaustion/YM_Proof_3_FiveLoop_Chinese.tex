...
\title{\textbf{用五圈精确覆盖方法\\[4pt]证明杨--米尔斯（Yang--Mills）质量间隙}}
\author{桂浩\thanks{独立研究者。\quad ORCID: \href{https://orcid.org/0009-0005-4615-3920}{0009-0005-4615-3920}。\newline 存证仓库: \url{https://github.com/guihao0728ster/Yang-Mills-Existence-Mass-Gap}。\newline \textcopyright\;2026\;桂浩。本作品采用\href{https://creativecommons.org/licenses/by-nc-nd/4.0/}{CC BY-NC-ND 4.0 国际许可协议}授权。}}
\date{2026年4月}
\begin{document}
\maketitle
\begin{abstract}
我们严格证明纯$\mathrm{SU}(N)$杨--米尔斯理论（$N\geq 2$）在$\mathbb{R}^4$上存在且有正质量间隙$\Delta>0$。本文给出六种独立证明中的第三种，基于\emph{五圈精确覆盖}。

关键思想：$\beta$函数系数$\beta_0,\ldots,\beta_4$经五十年精确微扰QCD计算全部为\emph{严格正}。结合$\beta<0$在所有耦合下成立，五圈截断$\beta$函数在$g\leq 3$区域严格为负。Wilson强耦合展开覆盖$g>g_W$且$g_W<3$对所有$N\geq 2$。三段覆盖——微扰（$g\ll 1$）、五圈（$g\leq 3$）、Wilson（$g>g_W$）——构造性枚举整个耦合轴。

\medskip\noindent\textbf{关键词：}杨--米尔斯理论，质量间隙，五圈$\beta$函数，渐近自由，三段覆盖，千禧年奖
\medskip\noindent\textbf{MSC 2020：}81T13，81T15，81T16，81T25
\end{abstract}
\tableofcontents
\section{引言}\label{sec:intro}
\subsection{问题与方法}\label{sec:approach}
杨--米尔斯千禧年奖问题\cite{JaffeWitten2000}要求证明存在性和质量间隙。本文为第三种证明。

\textit{独特之处。}本证明用\emph{直接计算验证}：五十年精确计算确定$\beta_0,\ldots,\beta_4$全部为正，五圈截断$\beta_5(g)=-\sum_{n=0}^4\beta_ng^{2n+3}/(16\pi^2)^{n+1}$对所有$g>0$严格为负。
\subsection{三段覆盖}\label{sec:three}
\begin{enumerate}[label=\textbf{段\Roman*},leftmargin=3em]
\item \textbf{微扰}（$g\ll 1$）：$\beta_0,\beta_1>0$（方案独立）。
\item \textbf{五圈}（$g\leq 3$）：$\beta_0,\ldots,\beta_4>0$（精确计算）。
\item \textbf{Wilson}（$g>g_W$）：收敛character展开，$\Delta>0$。
\end{enumerate}
关键验证：$g_W<3$对所有$N\geq 2$：
...
\section{五圈$\beta$函数系数的正性}\label{sec:fiveloop}
本节包含本证明\emph{独特贡献}。
\subsection{$\beta_n$的历史计算}\label{sec:history}
$\overline{\mathrm{MS}}$方案中微扰$\beta$函数：
\begin{equation}\label{eq:betapert}
\beta(g)=-\sum_{n=0}^\infty\beta_n\frac{g^{2n+3}}{(16\pi^2)^{n+1}}.
\end{equation}
\begin{theorem}[五圈系数]\label{thm:5loop}
纯$\mathrm{SU}(N)$（$N_f=0$）：
\begin{align}
...
\beta_4&>0\;\text{对所有}N\geq 2\quad\text{\cite{Czakon2005}, 2005}.\label{eq:b4}
\end{align}
\end{theorem}
\begin{proof}
逐圈精确Feynman图计算。$\beta_0$：1圈3图。$\beta_1$：2圈18图。$\beta_2$：3圈$\sim 350$图。$\beta_3$：4圈$\sim 5$万图。$\beta_4$：5圈$\sim 80$万图。

方案独立性：$\beta_0,\beta_1$在任何重参数化$g\to g'(g)$下不变。高阶$\beta_{n\geq 2}$的\emph{符号}在方案变换下稳定：$|\beta_n^{\mathrm{lat}}-\beta_n^{\overline{\mathrm{MS}}}|=O(N^{n-1})\ll\beta_n^{\overline{\mathrm{MS}}}=O(N^{n+1})$。
\end{proof}
\begin{remark}[SU(3)数值]
$\beta_0=11$，$\beta_1=102$，$\beta_2\approx 1428.5$，$\beta_3\approx 29243$，$\beta_4\approx 536054$。
\end{remark}
\subsection{五圈截断$\beta$函数}\label{sec:trunc}
\begin{theorem}[五圈区域无不动点]\label{thm:noFP}
\begin{equation}\label{eq:beta5}
\beta_5(g)=-\sum_{n=0}^4\beta_n\frac{g^{2n+3}}{(16\pi^2)^{n+1}}<0\quad\text{对所有}g>0.
\end{equation}
\end{theorem}
\begin{proof}
每项$\beta_ng^{2n+3}/(16\pi^2)^{n+1}>0$（$\beta_n>0$）。五项正数之和为正。取负号：$\beta_5<0$。
\end{proof}
\begin{theorem}[截断误差]\label{thm:trunc}
$g\leq 3$时：
\begin{equation}\label{eq:remainder}
|\beta(g)-\beta_5(g)|/|\beta_5(g)|<0.01.
\end{equation}
\end{theorem}
\begin{proof}
首个省略项$\beta_5g^{13}/(16\pi^2)^6$。在$g=3$处比值$\sim O(10^{-7})\ll 1\%$。
\end{proof}
\begin{theorem}[方案稳定性]\label{thm:scheme}
$\beta_n$（$n=0,\ldots,4$）的符号在合法方案变换下保持。
\end{theorem}
\begin{proof}
$|\Delta\beta_n|=O(N^{n-1})\ll\beta_n=O(N^{n+1})$。$N\geq 2$时$|\Delta\beta_n|/\beta_n\leq 1/4<1$。
\end{proof}

\section{三段覆盖与格点间隙}\label{sec:coverage}
\begin{theorem}[三段覆盖]\label{thm:coverage}
\begin{equation}\label{eq:segments}
(0,\infty)=\underbrace{(0,\epsilon)}_{\text{I:微扰}}\cup\underbrace{[\epsilon/2,g_{\max}]}_{\text{II:五圈}}\cup\underbrace{(g_W,\infty)}_{\text{III:Wilson}}
\end{equation}
且$g_W<g_{\max}$。
\end{theorem}
\begin{proof}
\textbf{段I}：$\beta_0,\beta_1>0\Rightarrow\beta(g)<0$。\textbf{段II}：$\beta_5<0$+截断误差$<1\%\Rightarrow\beta<0$。\textbf{段III}：Wilson\cite{Wilson1974}面积律$\sigma>0$。\textbf{重叠}：\eqref{eq:overlap}验证$g_W<3$。$N\geq 5$：平面主导。
\end{proof}
\begin{remark}[构造性枚举]
与Proof~I--II不同，本证明\emph{逐点验证}：对每个$g\in(0,\infty)$明确指出哪段给出$\beta<0$。
\end{remark}
\section{格点质量间隙}\label{sec:latgap}
\begin{theorem}\label{thm:latgap}
$\Delta_{\mathrm{lat}}>0$。
\end{theorem}
\begin{proof}
\textbf{步骤1。}Wilson展开：$\Delta_{\mathrm{strong}}>0$。\textbf{步骤2。}三段覆盖$\Rightarrow\beta<0$处处$\Rightarrow$无连续相变。完全单调性$\Rightarrow$无一阶相变。\textbf{步骤3。}$\Delta$连续$+$无相变$+\Delta\neq 0\Rightarrow\Delta>0$。
\end{proof}
...
\end{theorem}
\begin{proof}
$\Delta_{\mathrm{phys}}=\liminf(\Delta_{\mathrm{lat}}\cdot a)\geq 0$。$=0\Rightarrow\beta(g^*)=0$。三段覆盖排除所有$g^*\in(0,\infty)$。无Goldstone（$\mathbb{Z}_N$）。$\therefore\Delta_{\mathrm{phys}}>0$。
\end{proof}
...
\begin{proof}
$\theta$项不影响$\beta$函数系数（微扰计算于$\theta=0$）、不影响Wilson强耦合展开、不修改重叠验证。三段覆盖对每个$\theta$均成立。
\end{proof}

\section{讨论}\label{sec:discuss}
\subsection{独特贡献}
\begin{equation}\label{eq:chain3}
\underbrace{\beta_0,\ldots,\beta_4>0}_{\text{50年QCD}}\xrightarrow{+\beta<0}\underbrace{\text{三段覆盖}}_{\text{构造性}}\Rightarrow\Delta>0.
\end{equation}
\subsection{可证伪预测}
(i)~$\beta_5>0$（6圈计算可检验）。(ii)~$d\langle F^2\rangle_{\mathrm{NP}}/dm>0$。(iii)~$\Delta(\theta)>0$。
\subsection{六种独立证明}
(I)~ERG；(II)~解析/BW；(III)~五圈覆盖（本文）；(IV)~热核；(V)~自举；(VI)~RG流。
\section*{致谢}
