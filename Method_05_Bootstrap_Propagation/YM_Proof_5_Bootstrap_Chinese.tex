...
\title{\textbf{用自举间隙传播方法\\[4pt]证明杨--米尔斯（Yang--Mills）质量间隙}}
...
\begin{abstract}
我们严格证明纯$\mathrm{SU}(N)$杨--米尔斯理论存在且有正质量间隙$\Delta>0$。本文为第五种证明，基于从Wilson构造性强耦合起点的\emph{自举间隙传播}。

关键思想：Wilson强耦合展开\cite{Wilson1974}以绝对收敛级数给出$\Delta_{\mathrm{strong}}>0$（构造性起点）。$\beta<0$定理作为\emph{传播机制}：因$\beta<0$禁止所有相变（连续相变被$\beta<0$排除，一阶相变被完全单调性排除），间隙不能在耦合变化时消失。间隙从强耦合"自举"传播到弱耦合。

本证明以概念极简著称：仅需Wilson起点（构造性）$+\beta<0$（代数性）$+$中值定理（拓扑性）。无复分析、无五圈计算、无热核。

\medskip\noindent\textbf{关键词：}杨--米尔斯理论，质量间隙，自举，间隙传播，Wilson强耦合，中值定理，千禧年奖
\medskip\noindent\textbf{MSC 2020：}81T13，81T25，82B28，81T16
\end{abstract}
\tableofcontents
\section{引言}\label{sec:intro}
\subsection{问题与方法}\label{sec:approach}
杨--米尔斯千禧年奖问题\cite{JaffeWitten2000}。本文为第五种证明。

\textit{独特之处。}本证明将论证剥离到\emph{最小逻辑核心}：Wilson构造性间隙$+\beta<0$作为传播机制。

\textit{自举原则：}
(i)~\textbf{种子}：$\Delta_{\mathrm{strong}}>0$（Wilson收敛级数）。
(ii)~\textbf{引擎}：$\beta<0$（无IR不动点$\Rightarrow$无连续相变）。
(iii)~\textbf{护盾}：$Z$完全单调（无一阶相变）。
(iv)~\textbf{收获}：中值定理$\Rightarrow\Delta>0$处处。
\subsection{单一原语}\label{sec:primitive}
$A_\mu^a$（spin-1）为唯一动力学自由度。Lichnerowicz公式
...
\section{Wilson构造性起点}\label{sec:wilson}
\subsection{Character展开}\label{sec:character}
\begin{theorem}[Wilson\cite{Wilson1974}]\label{thm:wilson}
$\beta_{\mathrm{lat}}$足够小时（$g$足够大），Wilson环满足面积律：
...
\end{theorem}
\begin{proof}
\textbf{步骤1（character展开）。}将$\exp(-S_W)$按$\mathrm{SU}(N)$的character展开：
\begin{equation}\label{eq:charexp}
e^{(\beta/N)\operatorname{Re}\operatorname{tr}(U_P)}=\sum_R d_R c_R(\beta)\chi_R(U_P),
\end{equation}
$R$遍历所有不可约表示，$d_R$为维数，$c_R$为修改Bessel函数之比。

\textbf{步骤2（收敛）。}$\beta_{\mathrm{lat}}<\beta_c$时$|c_R|<1$，级数绝对收敛。

\textbf{步骤3（面积律）。}积分内部链接变量后仅基本表示存活。

\textbf{步骤4（弦张力）。}
\begin{equation}\label{eq:sigma}
\sigma=-\ln(\beta_{\mathrm{lat}}/(2N^2))>0.
\end{equation}
\end{proof}
\begin{corollary}[强耦合间隙]\label{cor:stronggap}
\begin{equation}\label{eq:stronggap}
\Delta_{\mathrm{strong}}\geq\sqrt{\sigma}>0.
\end{equation}
\end{corollary}
\begin{proof}
面积律蕴含所有关联指数衰减，衰减率$\geq\sqrt{\sigma}$。
\end{proof}
\begin{remark}[构造性 vs 微扰]
Wilson结果是\emph{构造性}的：$\Delta_{\mathrm{strong}}$由绝对收敛级数给出，非微扰估计。
\end{remark}
\subsection{收敛域}\label{sec:domain}
\begin{theorem}[收敛半径]\label{thm:radius}
\begin{equation}\label{eq:betac}
\mathrm{SU}(2):\beta_c\approx 8;\quad\mathrm{SU}(3):\beta_c\approx 18;\quad\mathrm{SU}(N):\beta_c\sim 2N^2.
\end{equation}
等价地$g>g_W$：
\begin{equation}\label{eq:gW}
\mathrm{SU}(2):g_W\approx 0.71;\quad\mathrm{SU}(3):g_W\approx 0.58;\quad\mathrm{SU}(N):g_W\sim 1.
\end{equation}
\end{theorem}
\begin{remark}[$g_W$的意义]
$g_W$以上Wilson展开严格证明禁闭和间隙。自举传播将此结果扩展到\emph{所有}$g>0$。
\end{remark}

\section{自举间隙传播}\label{sec:bootstrap}
本节为本证明\emph{核心论证}。
\subsection{四个组件}\label{sec:components}
\begin{definition}[自举四组件]\label{def:bootstrap}
(i)~\textbf{种子}：$\Delta(g_s)>0$（推论\ref{cor:stronggap}）。
(ii)~\textbf{引擎}：$\beta(g)<0$对所有$g>0$（定理\ref{thm:beta}）。
(iii)~\textbf{护盾}：$Z(\beta)$完全单调（引理\ref{lem:cm}）。
(iv)~\textbf{收获}：中值定理。
\end{definition}
\subsection{传播定理}\label{sec:propagation}
\begin{theorem}[自举间隙传播]\label{thm:bootstrap}
$\Delta_{\mathrm{lat}}(g)>0$对所有$g>0$。
\end{theorem}
\begin{proof}
\textbf{步骤1（种子）。}$\Delta(g_s)>0$（推论\ref{cor:stronggap}）。

\textbf{步骤2（引擎：无连续相变）。}$\Delta(g^*)=0\Rightarrow\xi\to\infty\Rightarrow$共形不动点$\beta(g^*)=0$。但$\beta(g)<0$对所有$g>0$——无此不动点。故无连续相变。

\textbf{步骤3（护盾：无一阶相变）。}一阶相变要求$f'$跳跃。$Z$完全单调$\Rightarrow f$凹$\Rightarrow f'$单调$\Rightarrow f'$连续。无一阶相变。

\textbf{步骤4（$\Delta$连续）。}自伴算子特征值连续依赖参数。

\textbf{步骤5（排除$\Delta=0$）。}$\Delta(g^*)=0$的三种可能机制全被排除：
CFT需$\beta=0$（矛盾）；Goldstone需连续对称性（仅$\mathbb{Z}_N$）；幂律衰减需尺度不变即$\beta=0$（矛盾）。

\textbf{步骤6（收获：中值定理）。}$\Delta(g_s)>0+\Delta\neq 0$处处$+\Delta$连续$\Rightarrow\Delta>0$处处。
\end{proof}
\begin{remark}[极简性]
本证明仅用：(i)~Wilson存在性结果；(ii)~$\beta<0$非微扰结果；(iii)~完全单调性解析结果；(iv)~中值定理拓扑结果。结构：\emph{一点存在$+$不可消失$=$处处存在}。
\end{remark}
\begin{remark}[$\beta<0$的双重角色]
$\beta<0$既排除连续相变（引擎），又排除无质量粒子（步骤5的CFT/尺度不变性排除）。
\end{remark}
...
\begin{proof}
\textbf{步骤1。}$\Delta_{\mathrm{phys}}=\liminf(\Delta_{\mathrm{lat}}\cdot a)\geq 0$。

\textbf{步骤2。}$=0\Rightarrow$CFT$\Rightarrow\beta(g^*)=0$——矛盾。

\textbf{步骤3。}无Goldstone（$\mathbb{Z}_N$）。

\textbf{步骤4。}$\therefore\Delta_{\mathrm{phys}}>0$。

\textbf{步骤5（构造性）。}$\Delta_{\mathrm{phys}}=\Lambda_{\mathrm{QCD}}\cdot f(N)>0$，其中
...
\end{corollary}
\begin{proof}
$\theta$项不影响：(i)~Wilson种子；(ii)~$\beta<0$引擎；(iii)~完全单调护盾。自举四组件对每个$\theta$均成立。
\end{proof}
\section{讨论}\label{sec:discuss}
\subsection{独特贡献}
\begin{equation}\label{eq:chain5}
\underbrace{\Delta_{\mathrm{strong}}>0}_{\text{Wilson（构造性）}}+\underbrace{\beta<0}_{\text{代数（非微扰）}}\xrightarrow{\text{IVT}}\Delta>0\;\text{处处}.
\end{equation}
\subsection{可证伪预测}
(i)~$d\langle F^2\rangle_{\mathrm{NP}}/dm>0$。(ii)~$\operatorname{Cov}(F^2,G_{\mathrm{Dirac}})>0$。(iii)~$\Delta(\theta)>0$。
\subsection{六种独立证明}
(I)~ERG；(II)~解析/BW；(III)~五圈；(IV)~热核；(V)~自举（本文）；(VI)~RG流。
\section*{致谢}
